...
### ✅ 랜딩페이지 언어선택 2단계 동작 구현
**작업 시간:** 1시간

**배경:** 모바일에서 언어 스크롤 시 의도치 않게 바로 실행되는 문제

**완료 작업:**
1. **2단계 동작 구현**
   - 1단계: 첫 터치 → 언어 선택 (하이라이트만)
   - 2단계: 같은 언어 다시 터치 → 실행 (페이지 전환)
   - `window.__selectedLang` 상태로 선택 관리

2. **시각적 피드백 개선**
   - 선택 시 **1.08배 크기 확대** + 바운스 애니메이션
   - **흰색 테두리 (3px)** 추가
   - 그림자 효과로 튀어나온 느낌
   - 배경: 연한 파란색 유지 (제미니 블루 배경에서 구분됨)

3. **중복 함수 정의 버그 수정**
   - 문제: `window.selectLanguage`가 상단(512줄)과 하단(1582줄)에 중복 정의
   - 하단 함수가 상단 2단계 버전을 덮어써서 즉시 실행됨
   - 해결: 하단 정의 제거, 상단 2단계 버전만 사용

**CSS 코드:**
```css
.language-item.ready-to-start {
    ...
    animation: bounce-ready 0.6s ease-out;
}
...
**수정 파일:**
- `public/index.html` - 2단계 동작 스크립트 + CSS 애니메이션

---

### ✅ V1 공유페이지 전체 재생성 (TTS 업데이트)
**작업 시간:** 30초 (자동화)

**완료 작업:**
- `/api/admin/regenerate-all` API로 39개 공유페이지 일괄 재생성
- 결과: **성공 39개 / 실패 0개**
- 새 TTS 로직 (`<font>` 태그 추출, 500ms 대기) 적용
- 구버전 HTML도 최신 템플릿으로 업데이트
...
2. **언어 선택 UI 변경**
   - 변경 전: 드롭다운 (7개 전체 노출)
   - 변경 후: 스크롤 리스트 (3개 보이고 나머지 스크롤)
   - 높이: 160px 고정

3. **언어 순서 최종 확정**
   - 한국어 → 영어 → 프랑스어 → 중국어 → 일본어 → 스페인어 → 독일어

**수정 파일:**
- `public/index.html` - 랜딩페이지 레이아웃
...
**인포그래픽 내용:**
- 3단계로 완성하는 나만의 AI 여행 가이드
- 1단계: 준비 및 사전 학습
- 2단계: AI 해설 요청 및 저장
- 3단계: 나만의 가이드북 제작 및 공유

**수정 파일:**
- `public/index.html` - 인포그래픽 모달 HTML
- `public/index.js` - 모달 이벤트 핸들러
- `public/images/infographic-feature1.png` - 인포그래픽 이미지
...
**완료 작업:**
1. **카드 클릭 시 동영상 모달**
   - 전체 화면 오버레이 (bg-black/90)
   - "영상 보기" 힌트 텍스트 추가
   - playsinline 속성 (모바일 인라인 재생)
...
3. **버그 수정**
   - autoplay 속성 제거 (언어 선택 시 자동 재생 버그)
   - controls 속성 제거 (깔끔한 UI)

**수정 파일:**
- `public/index.html` - 동영상 모달 HTML
- `public/index.js` - 동영상 모달 이벤트 핸들러
- `public/images/feature-video.mp4` - 기능 소개 동영상
